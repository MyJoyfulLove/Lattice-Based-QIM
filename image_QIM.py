...
def packing_radius(base: np.array) -> float:
    """
    Calculate the packing radius of lattice
    :param base: lattice base
    :return: packing radius
    """
    return round(min(pow(sum(pow(base, 2)), 1 / 2)) / 2, 5)
...
if __name__ == '__main__':
    ...
    for num in range(0, len(carrier), N):
        ...
        prd += sum(pow(carrier[num: num + N], 2))
        count += 1
        pass
    ...
    print('PSNR of QIM')
    print(PSNR_standard)
    PRD_standard = math.sqrt(mse_standard / prd)
    print('PRD of QIM')
    print(PRD_standard)
